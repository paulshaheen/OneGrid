...
az deployment group create -g pcpro-poc-rg -f planetary-computer-pro-poc/deploy/azure/main.bicep
```
...
Copy-paste starting points live in [`planetary-computer-pro-poc/samples/`](planetary-computer-pro-poc/samples/):

- [`planetary-computer-pro-poc/samples/sample-assets.csv`](planetary-computer-pro-poc/samples/sample-assets.csv) — 11 Gulf of Mexico point assets
  (platforms, wells, LNG terminal, refinery, storage, port) with the full column set.
- [`planetary-computer-pro-poc/samples/sample-assets.geojson`](planetary-computer-pro-poc/samples/sample-assets.geojson) — a `FeatureCollection`
...
| `planetary-computer-pro-poc/deploy/azure/teardown.sh` | Deletes the resource group and everything in it |
| `OGE-OneGrid/` | The OneGrid accelerator: Fabric lakehouse/eventhouse/semantic-model artifacts, the report-app + webapp + chatagent web application, and its own deploy tooling |
...
```bash
./planetary-computer-pro-poc/deploy/azure/teardown.sh pcpro-poc-rg
```
